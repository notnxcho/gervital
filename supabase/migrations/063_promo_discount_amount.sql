-- ════════════════════════════════════════════════════════════════════════════
-- 063_promo_discount_amount.sql
-- 1. promotions.discount_amount: ahorro REAL de la promo (solo asistencia).
--    El KPI "descuento otorgado" no puede reconstruirlo desde paid_amount porque
--    ese total incluye transporte (sin descuento); se guarda al crear la promo.
-- 2. create_prepaid_promo: recrea con SET search_path = public (hardening, alinea
--    con migraciones definer recientes) y calcula/guarda discount_amount.
-- month es 0-indexed.
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT COALESCE(SUM(paid_amount), 0),
         -- ahorro real: solo asistencia. attendance_chargeable_gross es el monto YA
         -- descontado; el bruto sin dto = /(1 - pct/100); ahorro = bruto - descontado.
         COALESCE(SUM(
           CASE WHEN discount_percent > 0 AND discount_percent < 100
             THEN attendance_chargeable_gross / (1 - discount_percent / 100.0) - attendance_chargeable_gross
             ELSE 0 END
         ), 0)
  ...
  UPDATE promotions
  SET paid_amount = v_total, discount_amount = ROUND(v_discount)
  WHERE id = v_promo_id;

  RETURN jsonb_build_object('success', true, 'promoId', v_promo_id,
    'monthsUpdated', v_range_count, 'paidAmount', v_total, 'discountAmount', ROUND(v_discount));
END;
