-- ============================================================================
-- 025: Add "intolerante a la lactosa" medical flag
-- ----------------------------------------------------------------------------
-- - medical_info.is_lactose_intolerant boolean
-- - clients_full: add isLactoseIntolerant to the medicalInfo jsonb
-- - create/update_client_full: add p_med_is_lactose_intolerant. Old signatures
--   dropped to avoid overload accumulation ("function is not unique").
-- ============================================================================

ALTER TABLE medical_info
  ADD COLUMN IF NOT EXISTS is_lactose_intolerant BOOLEAN NOT NULL DEFAULT FALSE;

-- clients_full view (024 def + isLactoseIntolerant) -------------------------
...
-- RPCs: drop current signatures, recreate with the new flag -------------------
DROP FUNCTION IF EXISTS public.create_client_full(text, text, text, text, date, text, date, integer, text, boolean, text[], text, text, text, text, text, text, text, text, text, text, text, boolean, boolean, boolean, text, text, jsonb, text);
DROP FUNCTION IF EXISTS public.update_client_full(uuid, text, text, text, text, date, text, date, integer, text, boolean, text[], text, text, text, text, text, text, text, text, text, text, text, boolean, boolean, boolean, text, text, jsonb, text);
...
CREATE OR REPLACE FUNCTION public.create_client_full(
  ...
  p_med_is_lactose_intolerant boolean DEFAULT false
)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.update_client_full(
  ...
  p_emergency_contacts jsonb DEFAULT NULL::jsonb, p_transfer_responsible text DEFAULT NULL::text,
  p_med_is_lactose_intolerant boolean DEFAULT NULL::boolean
)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
